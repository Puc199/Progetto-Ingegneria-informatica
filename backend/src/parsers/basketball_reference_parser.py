from __future__ import annotations

import re
from typing import Optional

from bs4 import BeautifulSoup, Comment

from src.parsers.base_parser import (
    fetch_html_crawl4ai,
    make_soup,
    extract_page_title,
    build_result,
)


def _norm(s: str) -> str:
    return re.sub(r"\s+", " ", (s or "")).strip()


def _dedupe_lines(text: str) -> str:
    seen = set()
    out = []
    for line in (text or "").splitlines():
        line = _norm(line)
        if not line:
            continue
        key = line.lower()
        if key in seen:
            continue
        seen.add(key)
        out.append(line)
    return "\n".join(out).strip()


def _dedupe_blocks(blocks: list[str]) -> list[str]:
    seen = set()
    out = []
    for b in blocks:
        b = _dedupe_lines(b)
        if not b:
            continue
        key = b.lower()
        if key in seen:
            continue
        seen.add(key)
        out.append(b)
    return out


def _strip_noise(soup: BeautifulSoup) -> BeautifulSoup:
    for tag in soup.find_all(["script", "style", "noscript", "svg", "template"]):
        tag.decompose()
    for sel in [
        "nav", "header", "footer", "aside",
        "#header", "#footer", "#nav", "#srcom", "#site_menu", "#footer_wrapper",
        ".footer", ".nav", ".breadcrumbs", ".ad-placeholder", ".advertisement",
        ".overlay", ".popup", ".cookie", ".cookies", ".sr_ad", ".promo"
    ]:
        for tag in soup.select(sel):
            tag.decompose()
    return soup


def _guess_title(soup: BeautifulSoup) -> Optional[str]:
    for sel in ["h1", "title", 'meta[property="og:title"]']:
        node = soup.select_one(sel) if sel.startswith(("meta[", "h1", "title")) else None
        if sel == "h1":
            node = soup.find("h1")
            if node:
                t = _norm(node.get_text(" ", strip=True))
                if t:
                    return t
        elif sel == "title":
            node = soup.find("title")
            if node:
                t = _norm(node.get_text(" ", strip=True))
                if t:
                    return t
        else:
            if node and node.get("content"):
                t = _norm(node["content"])
                if t:
                    return t
    t = extract_page_title(soup)
    return _norm(t) if t else None


def _text_from(node) -> str:
    if not node:
        return ""
    clone = BeautifulSoup(str(node), "html.parser")
    for bad in clone.select("script, style, noscript, table, svg, template"):
        bad.decompose()
    return _dedupe_lines(clone.get_text("\n", strip=True))


def _tables_from_visible(soup: BeautifulSoup) -> str:
    blocks = []
    for table in soup.find_all("table"):
        cap = table.find("caption")
        lines = []
        if cap:
            ct = _norm(cap.get_text(" ", strip=True))
            if ct:
                lines.append(ct)
        for tr in table.find_all("tr"):
            cells = [ _norm(c.get_text(" ", strip=True)) for c in tr.find_all(["th", "td"]) ]
            cells = [c for c in cells if c]
            if cells:
                lines.append(" | ".join(cells))
        if lines:
            blocks.append("\n".join(lines))
    return "\n\n".join(_dedupe_blocks(blocks)).strip()


def _tables_from_comments(html: str) -> str:
    outer = BeautifulSoup(html, "html.parser")
    blocks = []
    for c in outer.find_all(string=lambda x: isinstance(x, Comment)):
        txt = str(c)
        if "<table" not in txt.lower() and "stats_table" not in txt.lower():
            continue
        csoup = BeautifulSoup(txt, "html.parser")
        for table in csoup.find_all("table"):
            cap = table.find("caption")
            lines = []
            if cap:
                ct = _norm(cap.get_text(" ", strip=True))
                if ct:
                    lines.append(ct)
            for tr in table.find_all("tr"):
                cells = [ _norm(x.get_text(" ", strip=True)) for x in tr.find_all(["th", "td"]) ]
                cells = [x for x in cells if x]
                if cells:
                    lines.append(" | ".join(cells))
            if lines:
                blocks.append("\n".join(lines))
    return "\n\n".join(_dedupe_blocks(blocks)).strip()


def _branch(url: str, html: str) -> str:
    u = (url or "").lower()
    h = (html or "").lower()
    if "/playoffs/" in u:
        return "playoffs"
    if "/teams/" in u:
        return "teams"
    if "/executives/" in u:
        return "executives"
    if "/players/" in u:
        return "players"
    if "assistant coaches and staff" in h or "injury report" in h:
        return "teams"
    return "generic"


def _player_parts(soup: BeautifulSoup, html: str) -> list[str]:
    parts = []
    for sel in ["#meta", "#info", "#content", "#leaderboard", "#all_all_stats", "#all_per_game", "#all_advanced"]:
        t = _text_from(soup.select_one(sel))
        if t:
            parts.append(t)

    special = [
        "#div_per_game", "#div_totals", "#div_advanced", "#div_shooting", "#div_adj_shooting",
        "#div_playoffs_per_game", "#div_playoffs_advanced", "#div_all_sims", "#div_transactions",
        "#div_faq", "#div_translations"
    ]
    for sel in special:
        for node in soup.select(sel):
            t = _text_from(node)
            if t:
                parts.append(t)

    comments = _tables_from_comments(html)
    if comments:
        parts.append(comments)

    visible = _tables_from_visible(soup)
    if visible:
        parts.append(visible)

    return parts


def _playoffs_parts(soup: BeautifulSoup, html: str) -> list[str]:
    parts = []
    for sel in ["#meta", "#content", "#all_series", "#all_games", "#all_playoffs"]:
        t = _text_from(soup.select_one(sel))
        if t:
            parts.append(t)
    visible = _tables_from_visible(soup)
    if visible:
        parts.append(visible)
    comments = _tables_from_comments(html)
    if comments:
        parts.append(comments)
    return parts


def _teams_parts(soup: BeautifulSoup, html: str) -> list[str]:
    parts = []
    for sel in ["#meta", "#content", "#all_roster", "#all_schedule", "#all_injury_report", "#all_staff"]:
        t = _text_from(soup.select_one(sel))
        if t:
            parts.append(t)
    visible = _tables_from_visible(soup)
    if visible:
        parts.append(visible)
    comments = _tables_from_comments(html)
    if comments:
        parts.append(comments)
    return parts


def _executives_parts(soup: BeautifulSoup, html: str) -> list[str]:
    parts = []
    for sel in ["#meta", "#content", "#all_transactions", "#all_franchises", "#all_awards", "#all_draft_picks"]:
        t = _text_from(soup.select_one(sel))
        if t:
            parts.append(t)
    visible = _tables_from_visible(soup)
    if visible:
        parts.append(visible)
    comments = _tables_from_comments(html)
    if comments:
        parts.append(comments)
    return parts


def parse_basketball_reference(url: str, html_text: Optional[str] = None, html: Optional[str] = None, raw_html: Optional[str] = None, content: Optional[str] = None, **kwargs):
    resolved = html_text or html or raw_html or content or kwargs.get("html_text") or kwargs.get("html") or kwargs.get("raw_html") or kwargs.get("content")
    if not resolved or resolved == "string":
        try:
            resolved = fetch_html_crawl4ai(url)
        except Exception:
            resolved = None

    domain = "www.basketball-reference.com"
    if not resolved:
        res = build_result(url, domain, None, None, "")
        res["branch"] = "generic"
        return res

    soup = make_soup(resolved)
    soup = _strip_noise(soup)
    title = _guess_title(soup)
    br = _branch(url, resolved)

    if br == "players":
        parts = _player_parts(soup, resolved)
    elif br == "playoffs":
        parts = _playoffs_parts(soup, resolved)
    elif br == "teams":
        parts = _teams_parts(soup, resolved)
    elif br == "executives":
        parts = _executives_parts(soup, resolved)
    else:
        parts = []
        for sel in ["#meta", "#content"]:
            t = _text_from(soup.select_one(sel))
            if t:
                parts.append(t)
        visible = _tables_from_visible(soup)
        if visible:
            parts.append(visible)
        comments = _tables_from_comments(resolved)
        if comments:
            parts.append(comments)

    parts = _dedupe_blocks([p for p in parts if p])
    parsed = "\n\n".join(parts).strip()
    if not parsed:
        parsed = _dedupe_lines(soup.get_text("\n", strip=True))

    res = build_result(url, domain, title, resolved, parsed)
    res["branch"] = br
    return res
