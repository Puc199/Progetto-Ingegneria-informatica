...
class MorningstarParser(BaseParser):
    """
    Parser di global.morningstar.com.

    Il sito ha due tipi di pagina che vanno trattati in modo opposto.

    Gli articoli sono testo giornalistico dentro una struttura stabile: le
    classi seguono la convenzione BEM di Morningstar ('mdc-story-body__mdc'),
    che non cambia fra un rilascio e l'altro, e limitarsi a quel contenitore
    esclude da sola le code editoriali ("More In...", "About the Author",
    "Securities Mentioned") senza bisogno di una blacklist.

    Le pagine di quotazione invece sono gusci: prezzi, rating e metriche
    arrivano via XHR dopo l'idratazione, quindi nell'HTML statico non c'e'
    niente. In quel caso il parser lo dichiara esplicitamente invece di
    restituire il menu di navigazione facendolo passare per contenuto — una
    pagina vuota che sembra piena e' peggio di un errore dichiarato.
    """
    ...
    def extract_blocks(self, soup, url: str, html: str) -> list[str]:
        ...
        if branch == "article":
            return _collect_article(soup) or _fallback_article(soup)
