...
class TradingViewParser(BaseParser):
    """
    Parser di it.tradingview.com.

    TradingView e' una single page application React: i nomi delle classi CSS
    contengono un hash di build e cambiano a ogni rilascio, quindi come
    selettori sono inutilizzabili. Il parser si aggancia invece agli attributi
    'data-qa-id', che il sito usa per i propri test automatici e che restano
    stabili nel tempo.

    La scelta di fondo e' una whitelist (KEEP_SECTIONS) e non una blacklist:
    la pagina contiene una quindicina di widget e la maggioranza e' rumore —
    gauge grafiche, idee degli utenti, elenchi di notizie di terze parti,
    domande frequenti generate da un modello fisso.
    """
    ...
    def extract_blocks(self, soup, url: str, html: str) -> list[str]:
        # Le pagine /news/ costruiscono l'elenco in JavaScript dopo il primo
        # paint: l'HTML salvato contiene solo segnaposto. Lo si dichiara invece
        # di restituire silenziosamente una pagina vuota.
        if _branch(url) == "news" and _looks_skeleton(soup):
            return [
