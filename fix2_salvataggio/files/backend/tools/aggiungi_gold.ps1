...
function Mostra-Stato {
    ...
    Write-Host ""
    Write-Host ("{0,-32} {1,>8} {2,>8} {3,>10}" -f "dominio", "scaricate", "con testo", "mancano")
    Write-Host ("-" * 62)
...
try {
    $esito = Invia-Json -Path "/add_gold_standard" -Corpo @{ url = $Url; gold_text = $testo }
} catch {
    $messaggio = $_.Exception.Message
    if ($_.ErrorDetails.Message) {
        try { $messaggio = ($_.ErrorDetails.Message | ConvertFrom-Json).detail } catch { $messaggio = $_.ErrorDetails.Message }
    }
    Write-Host ""
